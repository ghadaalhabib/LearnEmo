...
class MultiTaskVisionTransformer(nn.Module):
    """
    Wrapper for VisionTransformer with 4 separate classification heads for multi-task learning.
    """

    def __init__(self, backbone: VisionTransformer, num_classes_per_task=4):
        super().__init__()
        self.backbone = backbone
        dim = backbone.embed_dim  # 512 for dim512 model

        self.engagement_head = nn.Linear(dim, num_classes_per_task)
        self.boredom_head = nn.Linear(dim, num_classes_per_task)
        self.confusion_head = nn.Linear(dim, num_classes_per_task)
        self.frustration_head = nn.Linear(dim, num_classes_per_task)

    def forward(self, x):
        features = self.backbone.forward_features(x)  # Shape: (B, D)
        return {
            'engagement': self.engagement_head(features),
            'boredom': self.boredom_head(features),
            'confusion': self.confusion_head(features),
