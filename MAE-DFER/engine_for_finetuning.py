...
def train_class_batch(model, samples, target, criterion, task): # task='classification'):
    outputs = model(samples)

    if task == 'multitask':
        # target: [B, 4], outputs: dict of 4 [B, 4]
        loss = (
            criterion(outputs['engagement'], target[:, 0]) +
            criterion(outputs['boredom'], target[:, 1]) +
            criterion(outputs['confusion'], target[:, 2]) +
            criterion(outputs['frustration'], target[:, 3])
        )
    else:
        loss = criterion(outputs, target)
...
# End here    
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            loss, output = train_class_batch(
                model, samples, targets, criterion, task=args.task)
...
# Here
        ...
        if args.task == 'multitask':
            # Average accuracy over 4 heads
            class_acc = 0
            for i, key in enumerate(['engagement', 'boredom', 'confusion', 'frustration']):
                pred = output[key].argmax(dim=1)
                acc = (pred == targets[:, i]).float().mean()
                class_acc += acc / 4
        else:
            class_acc = (output.max(-1)[-1] == targets).float().mean()
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            if args.task == 'multitask':
                ...
                loss = (
                    criterion(output['engagement'], target[:, 0]) +
                    criterion(output['boredom'], target[:, 1]) +
                    criterion(output['confusion'], target[:, 2]) +
                    criterion(output['frustration'], target[:, 3])
                )
            
                accs = []
                for i, key in enumerate(['engagement', 'boredom', 'confusion', 'frustration']):
                    pred = output[key].argmax(dim=1)
                    acc = (pred == target[:, i]).float().mean()
                    accs.append(acc.item())
            
                acc1 = sum(accs) / 4
                acc5 = acc1  # no top-5 for 4-class ordinal labels
            else:
                loss = criterion(output, target)
