...
# here
from modeling_finetune import MultiTaskVisionTransformer
...
def get_args():
    ...
    # here
    parser.add_argument('--task', default='classification', choices=['classification', 'multitask'],
                    help='Specify if this is a multitask learning run (DAiSEE) or standard classification')
...
# Here
    ...
    # Create base model (no head)
if 'no_depth' in args.model and args.depth is not None:
    print(f"==> Note: use custom model depth={args.depth}!")
    base_model = create_model(
        args.model,
        pretrained=False,
        num_classes=0,  # base encoder only
        all_frames=args.num_frames * args.num_segments,
        tubelet_size=args.tubelet_size,
        drop_rate=args.drop,
        drop_path_rate=args.drop_path,
...
# Wrap with Multi-Task Head if needed
if args.task == 'multitask':
    model = MultiTaskVisionTransformer(base_model, num_classes_per_task=4)
else:
    model = base_model
    ...
    if args.finetune:
        ...
        # Remove old single-head if present
        for k in list(checkpoint_model.keys()):
            if 'head' in k:
                print(f"Removing pretrained head key: {k}")
                del checkpoint_model[k]       
